...
def _random_crop_flip_transform(img, mask, size=256):
    # Random crop
    i, j, h, w = F.RandomCrop.get_params(img, output_size=(size, size))
    ...
    img = F.to_tensor(img)          # shape (C,H,W)
    mask = torch.tensor(
        F.pil_to_tensor(mask),      # shape (1,H,W) in older torchvision
        dtype=torch.long
    ).squeeze(0)                    # final shape (H,W)
...
class VOCDatasetWrapper(Dataset):
    def __init__(self, root, year="2012", image_set="train", download=False,
        ...
        transform to both the image and the mask.
        """
    ...
    def __getitem__(self, idx):
        # Get PIL Image and target mask from VOC
...
def get_train_loader(height=512, width=512, augment=True, **kwargs): 
    ...
    ds = VOCDatasetWrapper(image_set='train', transform=transform, **ds_kws)
...
def get_valid_loader(height=512, width=512, **kwargs): 
    ...
    ds = VOCDatasetWrapper(image_set='valid', transform=transform, **ds_kws)
    return DataLoader(ds, **kwargs)
...
def get_test_loader(height=512, width=512, **kwargs): 
    ...
    ds = VOCDatasetWrapper(image_set='test', transform=transform, **ds_kws)
    return DataLoader(ds, **kwargs)
...
class VOCDataModule(pl.LightningDataModule):
    """
    Loads the VOC2012 segmentation dataset from torchvision.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # You could allow passing different transforms or define them here
        self.train_transform = lambda x, y: _random_crop_flip_transform(x, y, size=self.patch_size)
        self.val_transform   = lambda x, y: _random_crop_flip_transform(x, y, size=self.patch_size)
        self.test_transform  = lambda x, y: _random_crop_flip_transform(x, y, size=self.patch_size)

    def prepare_data(self):
        """
        Download the dataset. This is called only from a single GPU in a DDP scenario.
        """
        # We only need to trigger a download so that the dataset is
        # available in self.data_dir.  "train" and "val" splits suffice.
    ...
    def setup(self, stage: Optional[str] = None):
        """
        Split the dataset for train/val/test. Called on every GPU in a DDP scenario.
        """

        if stage == "fit" or stage is None:
            self.dataset_train = VOCDatasetWrapper(
                root=self.data_dir,
                year="2012",
                image_set="train",
                download=False,
            ...
            self.dataset_val = VOCDatasetWrapper(
                root=self.data_dir,
                year="2012",
                image_set="val",
                download=False,
        ...
        if stage == "test" or stage is None:
            # Using val as test for demonstration, since VOC official 'test' set
            # does not have publicly available labels for segmentation.
            self.dataset_test = VOCDatasetWrapper(
                root=self.data_dir,
                year="2012",
                image_set="val",
                download=False,
